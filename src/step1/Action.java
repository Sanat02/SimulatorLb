...
public class Action {
    ...
    private final List<String> listOfEvents = new ArrayList<>();
    private int speed;
    ...
    public void startAction() {
        ...
        while (city.getDistance() > 0) {
            ...
            listOfEvents.add(eventCause(choiceEvent));
        ...
        for (int i = 0; i < listOfProducts.size(); i++) {
            System.out.printf("Product name:%s, quality:%s, final price:%s%n", listOfProducts.get(i).getGoodType(),
                    listOfProducts.get(i).getQualityOfGood(), listOfProducts.get(i).getFinalPrice());
    ...
    private String eventCause(NameOfEvents choiceEvent) {
        if (choiceEvent.equals(NameOfEvents.ORDINARY_DAY)) {
            ...
            if (checkSpoiled == listOfProducts.size()) {
                ...
                return "Все товары испорчены!Останавливаемся!";
